...
Exclusão manual (`DELETE /api/instancias/[id]`, endpoint `DELETE /instance` da UAZAPI): remove a instância dos dois lados. A chamada à UAZAPI vem antes do delete local de propósito — se ela recusar, nada é apagado aqui e a instância continua utilizável; a ordem inversa deixaria uma instância órfã na UAZAPI, invisível pro escritório. A rota da UAZAPI não recebe id: a instância excluída é a dona do `uazapi_token` enviado no header (nunca o `admintoken`). Escrita restrita a owner/admin, como criar/reconectar.
